# Multi-Provider Architecture: Visual Guide

**Visual comparison of current vs proposed architecture**

---

## Before: Single Provider (Current)

```
┌─────────────────────────────────────────────────────────────────┐
│                         CLIENT REQUEST                           │
│                    POST /api/generate                            │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                         API ROUTES                               │
│                    (routes/api.js)                               │
│                                                                   │
│  • Parse request body                                            │
│  • Validate input                                                │
│  • Call docGenerator                                             │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                    DOC GENERATOR SERVICE                         │
│                  (services/docGenerator.js)                      │
│                                                                   │
│  const claudeClient = new ClaudeClient()  ◄── HARDCODED         │
│                                                                   │
│  • Parse code (AST analysis)                                     │
│  • Build prompts                                                 │
│  • Call claudeClient.generate()                                  │
│  • Calculate quality score                                       │
│  • Add attribution                                               │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                        CLAUDE CLIENT                             │
│                   (services/claudeClient.js)                     │
│                                                                   │
│  const model = 'claude-sonnet-4-5-20250929'  ◄── HARDCODED      │
│                                                                   │
│  • Wrap Anthropic SDK                                            │
│  • Handle retries (3x exponential backoff)                       │
│  • Extract error messages                                        │
│  • Manage prompt caching                                         │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                      ANTHROPIC SDK                               │
│                  (@anthropic-ai/sdk)                             │
│                                                                   │
│  this.client.messages.create()                                   │
│  this.client.messages.stream()                                   │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
                    ┌────────────────┐
                    │  CLAUDE API    │
                    │  (Only Option) │
                    └────────────────┘
```

**Problems:**
- ❌ Tightly coupled to Claude (can't switch providers)
- ❌ Model name hardcoded (line 8 of claudeClient.js)
- ❌ No provider abstraction
- ❌ Hard to test (can't mock provider easily)
- ❌ Can't leverage other models (GPT-5.1, Gemini, etc.)

---

## After: Multi-Provider (Proposed)

```
┌─────────────────────────────────────────────────────────────────┐
│                         CLIENT REQUEST                           │
│                    POST /api/generate                            │
│         Optional: X-LLM-Provider: openai                         │
│                   X-LLM-Model: gpt-5.1               │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                         MIDDLEWARE                               │
│                  (middleware/llmProvider.js)                     │
│                                                                   │
│  req.llmProvider = llmFactory.createFromEnv()                    │
│    OR override from headers (X-LLM-Provider)                     │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                         API ROUTES                               │
│                    (routes/api.js)                               │
│                                                                   │
│  const docGenerator = new DocGeneratorService(req.llmProvider)   │
│                                              ▲                    │
│                                              │                    │
│                                    DEPENDENCY INJECTION          │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                    DOC GENERATOR SERVICE                         │
│                  (services/docGenerator.js)                      │
│                                                                   │
│  constructor(llmProvider = null) {                               │
│    this.llmProvider = llmProvider || llmFactory.createFromEnv()  │
│  }                                                               │
│                                                                   │
│  • Parse code (same as before)                                   │
│  • Build prompts (same as before)                                │
│  • Call this.llmProvider.generate()  ◄── ABSTRACTED             │
│  • Calculate quality score (same)                                │
│  • Add attribution (same)                                        │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                         LLM FACTORY                              │
│                  (llm/factory/LLMFactory.js)                     │
│                                                                   │
│  createFromEnv() {                                               │
│    provider = process.env.LLM_PROVIDER  // 'anthropic' | 'openai'│
│    model = process.env.LLM_MODEL                                 │
│    return this.create({ provider, model, ... })                  │
│  }                                                               │
│                                                                   │
│  Registry:                                                       │
│    'anthropic' → ClaudeProvider                                  │
│    'openai' → OpenAIProvider                                     │
│    'azure-openai' → AzureOpenAIProvider                          │
│    'bedrock' → BedrockProvider                                   │
└────────────────────────────┬────────────────────────────────────┘
                             │
                    ┌────────┴────────┐
                    │  SELECT PROVIDER │
                    └────────┬────────┘
                             │
        ┌────────────────────┼────────────────────┬───────────────┐
        │                    │                    │               │
        ▼                    ▼                    ▼               ▼
┌──────────────┐     ┌──────────────┐     ┌──────────────┐     ┌──────────────┐
│   CLAUDE     │     │   OPENAI     │     │    AZURE     │     │   BEDROCK    │
│  PROVIDER    │     │  PROVIDER    │     │   OPENAI     │     │  PROVIDER    │
│              │     │              │     │  PROVIDER    │     │              │
│ extends      │     │ extends      │     │ extends      │     │ extends      │
│ BaseLLM      │     │ BaseLLM      │     │ BaseLLM      │     │ BaseLLM      │
│ Provider     │     │ Provider     │     │ Provider     │     │ Provider     │
└──────┬───────┘     └──────┬───────┘     └──────┬───────┘     └──────┬───────┘
       │                    │                    │                    │
       │ Implements:        │ Implements:        │ Implements:        │
       │ • generate()       │ • generate()       │ • generate()       │
       │ • streaming()      │ • streaming()      │ • streaming()      │
       │ • capabilities()   │ • capabilities()   │ • capabilities()   │
       │                    │                    │                    │
       ▼                    ▼                    ▼                    ▼
┌──────────────┐     ┌──────────────┐     ┌──────────────┐     ┌──────────────┐
│ Anthropic    │     │   OpenAI     │     │ Azure OpenAI │     │AWS Bedrock   │
│     SDK      │     │     SDK      │     │     SDK      │     │    SDK       │
└──────┬───────┘     └──────┬───────┘     └──────┬───────┘     └──────┬───────┘
       │                    │                    │                    │
       ▼                    ▼                    ▼                    ▼
┌──────────────┐     ┌──────────────┐     ┌──────────────┐     ┌──────────────┐
│  Claude API  │     │  OpenAI API  │     │  Azure API   │     │  Bedrock API │
└──────────────┘     └──────────────┘     └──────────────┘     └──────────────┘
```

**Benefits:**
- ✅ Provider-agnostic architecture
- ✅ Runtime provider switching (env vars or headers)
- ✅ Easy to add new providers
- ✅ Testable (inject mock providers)
- ✅ Cost optimization (use cheapest provider)
- ✅ Fallback chains (if Claude fails, try OpenAI)

---

## Provider Interface (Contract)

All providers must implement `BaseLLMProvider`:

```
┌─────────────────────────────────────────────────────────────────┐
│                       BaseLLMProvider                            │
│                     (Abstract Interface)                         │
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│  🔴 ABSTRACT METHODS (Must Implement)                            │
│                                                                   │
│  async generate(prompt, options)                                 │
│    → Returns: { text, metadata }                                 │
│    → Throws: LLMError                                            │
│                                                                   │
│  async generateWithStreaming(prompt, onChunk, options)           │
│    → Returns: { text, metadata }                                 │
│    → Throws: LLMError                                            │
│                                                                   │
│  getCapabilities()                                               │
│    → Returns: { streaming, caching, vision, ... }                │
│                                                                   │
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│  🟢 CONCRETE METHODS (Provided by Base)                          │
│                                                                   │
│  _validateConfig(config)                                         │
│  _executeWithRetry(fn, operation)                                │
│  supportsStreaming() → boolean                                   │
│  supportsCaching() → boolean                                     │
│  getProviderName() → string                                      │
│  getModel() → string                                             │
│  estimateTokens(text) → number                                   │
│                                                                   │
└─────────────────────────────────────────────────────────────────┘
```

**Example Usage:**
```javascript
// Any provider works the same way
const provider = llmFactory.createFromEnv()

const result = await provider.generate('Explain quantum computing', {
  systemPrompt: 'You are a helpful assistant.',
  enableCaching: true,
  maxTokens: 4000
})

console.log(result.text)  // Generated text
console.log(result.metadata)  // { provider, model, tokens, latency, ... }
```

---

## Configuration Flow

```
┌─────────────────────────────────────────────────────────────────┐
│                       ENVIRONMENT VARIABLES                      │
│                         (server/.env)                            │
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│  LLM_PROVIDER=anthropic         # or 'openai', 'azure-openai'   │
│  LLM_API_KEY=sk-ant-...         # API key                        │
│  LLM_MODEL=claude-sonnet-4...   # Model identifier               │
│  LLM_MAX_TOKENS=4000            # Max tokens per request         │
│  LLM_MAX_RETRIES=3              # Max retry attempts             │
│  LLM_ENABLE_CACHING=true        # Enable prompt caching          │
│                                                                   │
│  # Backward compatibility                                        │
│  CLAUDE_API_KEY=sk-ant-...      # Falls back to this if no key   │
│  OPENAI_API_KEY=sk-...                                           │
│                                                                   │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                         LLM CONFIG                               │
│                   (llm/config/LLMConfig.js)                      │
│                                                                   │
│  • Load environment variables                                    │
│  • Validate configuration                                        │
│  • Apply defaults                                                │
│  • Provider-specific options                                     │
│                                                                   │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                         LLM FACTORY                              │
│                   (llm/factory/LLMFactory.js)                    │
│                                                                   │
│  createFromEnv() {                                               │
│    config = LLMConfig.get()                                      │
│    ProviderClass = this.registry.get(config.provider)            │
│    return new ProviderClass(config)                              │
│  }                                                               │
│                                                                   │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                      PROVIDER INSTANCE                           │
│                   (ClaudeProvider, etc.)                         │
│                                                                   │
│  • Initialized with validated config                             │
│  • Ready to generate text                                        │
│  • Singleton (reused across requests)                            │
│                                                                   │
└─────────────────────────────────────────────────────────────────┘
```

**Runtime Override (via Headers):**
```
POST /api/generate
X-LLM-Provider: openai
X-LLM-Model: gpt-5.1

                     ↓

Middleware creates provider instance with header values
(overrides environment config for this request only)
```

---

## Error Handling Flow

```
┌─────────────────────────────────────────────────────────────────┐
│                     PROVIDER API CALL                            │
│          (Claude API, OpenAI API, Azure API, etc.)               │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
                      ❌ API ERROR
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                      PROVIDER ADAPTER                            │
│                  (_adaptError() method)                          │
│                                                                   │
│  • Parse provider-specific error format                          │
│  • Extract status code, error type, message                      │
│  • Convert to standard LLMError                                  │
│                                                                   │
│  Example (Claude):                                               │
│    status: 429 → errorType: 'RATE_LIMIT'                         │
│    status: 401 → errorType: 'AUTH'                               │
│    status: 500 → errorType: 'SERVER_ERROR'                       │
│                                                                   │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                         LLM ERROR                                │
│                   (llm/providers/base/LLMError.js)               │
│                                                                   │
│  new LLMError('Rate limit exceeded', {                           │
│    provider: 'claude',                                           │
│    errorType: 'RATE_LIMIT',                                      │
│    statusCode: 429,                                              │
│    retryAfter: 60                                                │
│  })                                                              │
│                                                                   │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                        RETRY HELPER                              │
│                 (llm/providers/base/RetryHelper.js)              │
│                                                                   │
│  if (error.isRetryable()) {                                      │
│    wait exponential backoff (2^attempt * 1000ms)                 │
│    retry up to 3 times                                           │
│  } else {                                                        │
│    throw immediately (auth errors, validation errors)            │
│  }                                                               │
│                                                                   │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
                  ✅ SUCCESS or ❌ MAX RETRIES
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                    DOC GENERATOR SERVICE                         │
│                                                                   │
│  catch (error) {                                                 │
│    if (error instanceof LLMError) {                              │
│      // Log with provider context                                │
│      console.error(`[${error.provider}] ${error.message}`)       │
│      throw error  // Re-throw for API route                      │
│    }                                                             │
│  }                                                               │
│                                                                   │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                         API ROUTE                                │
│                                                                   │
│  catch (error) {                                                 │
│    const status = error.statusCode || 500                        │
│    res.status(status).json(error.toJSON())                       │
│  }                                                               │
│                                                                   │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                       CLIENT RESPONSE                            │
│                                                                   │
│  {                                                               │
│    "error": "Claude API rate limit exceeded",                    │
│    "provider": "claude",                                         │
│    "errorType": "RATE_LIMIT",                                    │
│    "statusCode": 429,                                            │
│    "retryAfter": 60                                              │
│  }                                                               │
│                                                                   │
└─────────────────────────────────────────────────────────────────┘
```

**Error Type Classification:**

| Error Type | Retryable? | Examples |
|------------|------------|----------|
| `AUTH` | ❌ No | Invalid API key |
| `RATE_LIMIT` | ✅ Yes (with backoff) | 429 rate limit |
| `VALIDATION` | ❌ No | Invalid parameters |
| `TIMEOUT` | ✅ Yes | Request timeout |
| `NETWORK` | ✅ Yes | Connection failed |
| `SERVER_ERROR` | ✅ Yes | 5xx errors |
| `QUOTA_EXCEEDED` | ❌ No | Account billing issue |
| `MODEL_NOT_FOUND` | ❌ No | Invalid model name |
| `UNKNOWN` | ❌ No | Unclassified error |

---

## Caching Strategy (Claude Only)

```
┌─────────────────────────────────────────────────────────────────┐
│                       FIRST REQUEST                              │
│                  (with enableCaching: true)                      │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                      CLAUDE PROVIDER                             │
│                                                                   │
│  messages: [                                                     │
│    {                                                             │
│      role: 'system',                                             │
│      content: [{                                                 │
│        type: 'text',                                             │
│        text: 'You are a documentation generator...',             │
│        cache_control: { type: 'ephemeral' }  ◄── CACHE THIS     │
│      }]                                                          │
│    },                                                            │
│    {                                                             │
│      role: 'user',                                               │
│      content: 'Generate docs for: function add(a,b)...'          │
│    }                                                             │
│  ]                                                               │
│                                                                   │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                        CLAUDE API                                │
│                                                                   │
│  • Processes system prompt (2,000 tokens)                        │
│  • WRITES TO CACHE (cache_creation_input_tokens: 2000)           │
│  • Processes user message (500 tokens)                           │
│  • Generates response (1,000 tokens)                             │
│                                                                   │
│  Cost: 2,500 input tokens × $0.003/1K = $0.0075                 │
│        1,000 output tokens × $0.015/1K = $0.015                  │
│        Total: $0.0225                                            │
│                                                                   │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
                    ⏱️ CACHE TTL: 1 HOUR
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                      SECOND REQUEST                              │
│              (within 1 hour, same system prompt)                 │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                        CLAUDE API                                │
│                                                                   │
│  • READS FROM CACHE (cache_read_input_tokens: 2000)  ✅         │
│  • Processes new user message (500 tokens)                       │
│  • Generates response (1,000 tokens)                             │
│                                                                   │
│  Cost: 2,000 cached tokens × $0.0003/1K = $0.0006  ◄── 90% OFF  │
│        500 new input tokens × $0.003/1K = $0.0015                │
│        1,000 output tokens × $0.015/1K = $0.015                  │
│        Total: $0.0171 (saved $0.0054 = 24%)                      │
│                                                                   │
└─────────────────────────────────────────────────────────────────┘

Response metadata:
{
  "inputTokens": 500,
  "outputTokens": 1000,
  "cacheReadTokens": 2000,  ◄── Indicates cache hit!
  "cacheWriteTokens": 0,
  "wasCached": true
}
```

**Caching Savings at Scale:**

| Scenario | Without Caching | With Caching | Savings |
|----------|-----------------|--------------|---------|
| 1 request/day | $0.02/day | $0.02/day (first) + $0.017/day (rest) | ~15% |
| 100 requests/day | $2.25/day | $0.022 + $1.71 = $1.73/day | ~23% |
| 1,000 requests/day | $22.50/day | $0.022 + $17.10 = $17.12/day | ~24% |

**At 1,000 requests/day:** $162/month saved!

---

## File Structure Comparison

### Before (Current)
```
server/
├── src/
│   ├── services/
│   │   ├── claudeClient.js       ◄── Tightly coupled to Claude
│   │   ├── docGenerator.js       ◄── Imports claudeClient directly
│   │   ├── codeParser.js
│   │   └── qualityScorer.js
│   ├── routes/
│   │   └── api.js
│   └── middleware/
│       └── (no LLM middleware)
└── __tests__/
    ├── claudeClient.test.js
    └── docGenerator.test.js
```

### After (Proposed)
```
server/
├── src/
│   ├── services/
│   │   ├── llm/                  ◄── NEW: LLM abstraction layer
│   │   │   ├── providers/
│   │   │   │   ├── base/
│   │   │   │   │   ├── BaseLLMProvider.js    (200 lines)
│   │   │   │   │   ├── LLMError.js           (50 lines)
│   │   │   │   │   └── RetryHelper.js        (60 lines)
│   │   │   │   ├── anthropic/
│   │   │   │   │   └── ClaudeProvider.js     (250 lines)
│   │   │   │   ├── openai/
│   │   │   │   │   └── OpenAIProvider.js     (200 lines)
│   │   │   │   └── index.js
│   │   │   ├── factory/
│   │   │   │   └── LLMFactory.js             (150 lines)
│   │   │   ├── config/
│   │   │   │   └── LLMConfig.js              (100 lines)
│   │   │   └── index.js
│   │   ├── claudeClient.js       ◄── LEGACY wrapper (backward compat)
│   │   ├── docGenerator.js       ◄── UPDATED: Use injected provider
│   │   ├── codeParser.js         (unchanged)
│   │   └── qualityScorer.js      (unchanged)
│   ├── routes/
│   │   └── api.js                ◄── UPDATED: Inject provider
│   └── middleware/
│       └── llmProvider.js        ◄── NEW: Provider injection
└── __tests__/
    ├── llm/                      ◄── NEW: Provider tests
    │   ├── providers/
    │   │   ├── base/
    │   │   │   ├── BaseLLMProvider.test.js
    │   │   │   ├── LLMError.test.js
    │   │   │   └── RetryHelper.test.js
    │   │   ├── anthropic/
    │   │   │   └── ClaudeProvider.test.js
    │   │   └── openai/
    │   │       └── OpenAIProvider.test.js
    │   ├── factory/
    │   │   └── LLMFactory.test.js
    │   └── integration/
    │       ├── provider-switching.test.js
    │       └── streaming.test.js
    ├── claudeClient.test.js      (keep for legacy)
    └── docGenerator.test.js      (update with mocks)
```

**New Code Statistics:**
- Base classes: ~310 lines
- ClaudeProvider: ~250 lines (refactored from claudeClient)
- OpenAIProvider: ~200 lines (new)
- Factory + Config: ~250 lines
- Tests: ~1,500 lines
- **Total: ~2,510 lines** (~1,200 new + ~300 refactored + ~1,500 tests)

---

## Design Patterns Applied

### 1. Strategy Pattern
```
Context: DocGeneratorService
Strategy Interface: BaseLLMProvider
Concrete Strategies: ClaudeProvider, OpenAIProvider, AzureOpenAIProvider

DocGeneratorService doesn't care WHICH provider it uses,
just that it implements generate() and generateWithStreaming().
```

### 2. Factory Pattern
```
Factory: LLMFactory
Products: ClaudeProvider, OpenAIProvider, etc.

Factory encapsulates provider creation logic.
Client code just says "give me a provider" without knowing HOW it's created.
```

### 3. Adapter Pattern
```
Adaptee: Anthropic SDK (specific API)
Target: BaseLLMProvider (common interface)
Adapter: ClaudeProvider (translates Anthropic API to common interface)

Each provider ADAPTS its SDK's API to the common BaseLLMProvider interface.
```

### 4. Dependency Injection
```
Dependent: DocGeneratorService
Dependency: BaseLLMProvider (interface, not concrete class)

DocGeneratorService receives provider via constructor (not new ClaudeClient()).
Makes it testable (inject mock) and flexible (switch providers).
```

### 5. Singleton Pattern
```
LLMFactory maintains a singleton instance of each provider+model combination.
Prevents creating multiple SDK clients for the same provider.
Improves performance (connection pooling, etc.).
```

---

## Quick Reference Card

### Switching Providers

**Via Environment Variables (server/.env):**
```bash
# Use Claude (default)
LLM_PROVIDER=anthropic
LLM_API_KEY=$CLAUDE_API_KEY
LLM_MODEL=claude-sonnet-4-5-20250929

# Use OpenAI
LLM_PROVIDER=openai
LLM_API_KEY=$OPENAI_API_KEY
LLM_MODEL=gpt-5.1

# Use Azure OpenAI
LLM_PROVIDER=azure-openai
LLM_API_KEY=$AZURE_OPENAI_API_KEY
LLM_MODEL=gpt-4
AZURE_OPENAI_ENDPOINT=https://your-resource.openai.azure.com
```

**Via API Headers (per-request override):**
```bash
curl -X POST http://localhost:3000/api/generate \
  -H "Content-Type: application/json" \
  -H "X-LLM-Provider: openai" \
  -H "X-LLM-Model: gpt-5.1" \
  -d '{"code": "...", "docType": "README"}'
```

### Provider Capabilities Matrix

| Provider | Streaming | Caching | Vision | Max Context | Cost (input) | Cost (output) |
|----------|-----------|---------|--------|-------------|--------------|---------------|
| **Claude Sonnet 4.5** | ✅ Yes | ✅ Yes | ✅ Yes | 200K tokens | $0.003/1K | $0.015/1K |
| **OpenAI GPT-5.1** | ✅ Yes | ❌ No | ✅ Yes | 128K tokens | $0.01/1K | $0.03/1K |
| **Azure OpenAI** | ✅ Yes | ❌ No | ✅ Yes | 128K tokens | $0.01/1K | $0.03/1K |
| **AWS Bedrock (Claude)** | ✅ Yes | ⚠️ Partial | ✅ Yes | 200K tokens | $0.003/1K | $0.015/1K |

### Common Tasks

**Add a new provider:**
1. Create `llm/providers/{name}/{Name}Provider.js`
2. Extend `BaseLLMProvider`
3. Implement `generate()`, `generateWithStreaming()`, `getCapabilities()`
4. Register in `LLMFactory`: `this.registry.set('name', NameProvider)`
5. Write tests

**Test with different provider:**
```bash
LLM_PROVIDER=openai npm run dev
```

**Check which provider is active:**
```javascript
const provider = llmFactory.createFromEnv()
console.log(provider.getProviderName())  // 'claude' or 'openai'
console.log(provider.getModel())         // 'claude-sonnet-4-5-20250929'
```

**Mock provider in tests:**
```javascript
const mockProvider = {
  generate: jest.fn().mockResolvedValue({
    text: 'Mock documentation',
    metadata: { provider: 'mock', inputTokens: 100 }
  }),
  generateWithStreaming: jest.fn(),
  getProviderName: () => 'mock'
}

const docGenerator = new DocGeneratorService(mockProvider)
```

---

## Implementation Timeline

| Phase | Duration | Key Deliverables |
|-------|----------|------------------|
| **Phase 1-2** | 4-5 hours | Base abstraction + Claude refactor |
| **Phase 3** | 1-2 hours | Factory + Config |
| **Phase 4-5** | 2 hours | Update DocGenerator + API routes |
| **Phase 6** | 2 hours | Add OpenAI provider |
| **Phase 7** | 2 hours | Comprehensive testing |
| **Phase 8** | 1 hour | Documentation + deployment |
| **Total** | **12-14 hours** | Multi-provider support complete |

---

## Success Metrics

✅ **Functionality:**
- All existing tests pass (no breaking changes)
- Can switch providers via environment variable
- Can switch providers via API header
- Streaming works with all providers
- Error handling consistent across providers

✅ **Quality:**
- 95%+ test coverage for provider layer
- < 5% performance overhead
- Zero production errors

✅ **Developer Experience:**
- Adding new provider takes < 2 hours
- Clear documentation
- Easy to understand code structure

✅ **Cost Optimization:**
- Caching works with Claude (24% savings)
- Can route to cheaper providers when appropriate

---

**Ready to implement? See [MULTI-PROVIDER-IMPLEMENTATION-PROMPT.md](./MULTI-PROVIDER-IMPLEMENTATION-PROMPT.md) for step-by-step guide.**
