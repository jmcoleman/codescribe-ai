...
export function TierOverrideBanner({ override, onClear }) {
  ...
  if (!isVisible || !override || isDismissed) {
    return null;
  }
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm text-amber-900 dark:text-amber-100">
            <span className="font-semibold">Tier Override Active:</span>{' '}
            Viewing as <span className="font-semibold capitalize">{override.tier}</span> • {timeText}
          </p>
        </div>
